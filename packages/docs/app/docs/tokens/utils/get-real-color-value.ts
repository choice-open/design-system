...
export type ColorType = "semantic" | "extended-semantic" | "base-light" | "base-dark"
...
export function getRealColorValue(
  ...
  try {
    ...
    try {
      ...
      const tokenPath = colorKey.startsWith("color.") ? colorKey : `color.${colorKey}`
      ...
      // 如果找不到颜色，标记为无效
      console.warn(`Color "${colorKey}" not found in tokens for mode "${mode}"`)
      isValid = false
 ...
 * 按类别获取颜色
 */
export function getColorsByType(category: "background" | "text" | "border" | "icon"): string[] {
  try {
 ...
 * @returns 透明度值 (0-1)
 */
export function getColorAlpha(colorKey: string, mode: "light" | "dark" = "light"): number {
  try {
    const themeMode = mode === "light" ? "." : "dark"
    const tokenPath = colorKey.startsWith("color.") ? colorKey : `color.${colorKey}`
 ...
 * 获取颜色类别前缀列表
 */
export const COLOR_CATEGORIES = ["background", "text", "border", "icon"] as const
 ...
 * 检查颜色是否属于指定类别
 */
export function isColorInCategory(colorKey: string, category: ColorCategory): boolean {
