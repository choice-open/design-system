    ...
    <>
      <Section
        ...
        content={
          <>
            <p>
              Our spacing system is built on a 4px base unit, providing preset values from 0 to
              80px. Using the spacing() function, you can set spacing with numeric multipliers
              (e.g., spacing(4) = 16px) or semantic aliases (e.g., spacing(&apos;sm&apos;) = 640px).
              The system supports arbitrary values, offering flexibility while maintaining
              consistency.
            </p>
          </>
        }
          ...
          {Object.entries(spacingScale)
            ...
            .map(([key, value], index) => (
              <Fragment key={key}>
                ...
                <SpacingValueComponent>{spacing(key as SpacingValue)}</SpacingValueComponent>
                <SpacingValueComponent>
                  $spacing(
                ...
                />
                {index !== Object.entries(spacingScale).length - 1 && <SpacingDivider />}
              </Fragment>
            ))}
        </SpacingGrid>
      </Section>
