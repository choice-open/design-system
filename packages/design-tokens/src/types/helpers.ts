...
type BreakpointTokenKeys = Extract<keyof typeof tokens, `breakpoints.${string}`>
...
type ExtractColorPath<T extends string> = T extends `color.${infer Rest}` ? Rest : never
...
// 导出具体的颜色类型以供使用
export type { BackgroundColors, ForegroundColors, BoundaryColors, IconColors, StandardColors }
...
// 提取有效的键名
type ExtractSpacingKey<T extends string> = T extends `spacing.${infer Key}` ? Key : never
type ExtractBreakpointKey<T extends string> = T extends `breakpoints.${infer Key}` ? Key : never
...
export type LetterSpacingKey = ExtractFontKey<TypographyTokenKeys, "letterSpacings">
...
type ExtractShadowKey<T extends string> = T extends `shadows.${infer Key}` ? Key : never
...
type ExtractRadiusKey<T extends string> = T extends `radius.${infer Key}` ? Key : never
...
type ExtractZIndexKey<T extends string> = T extends `zindex.${infer Key}` ? Key : never
