 ...
 * 检查 token 是否存在
 */
export function tokenExists<T extends string>(tokenId: T): tokenId is T & TokenKey {
...
export function listTokens(): string[] {
  return Object.keys(tokens).filter((key): key is string => typeof key === "string")
}
...
export function tokenInfo<K extends TokenKey>(tokenId: K) {
  ...
  return {
    id: tokenId,
    modes: Object.keys(tokenData).filter((key): key is string => typeof key === "string"),
    data: tokenData,
  }
}
...
export function getToken(path: string, mode: ThemeMode = ".") {
  ...
  if (modeData === undefined) {
    ...
    if (defaultData === undefined) {
      throw new Error(`Token '${path}' has no data for mode '${mode}' or default mode`)
    }
    return defaultData
  }
...
export function dimensionTokenToCss(dimensionToken: any): string {
  if (!dimensionToken || typeof dimensionToken.value === "undefined" || !dimensionToken.unit) {
