...
import type { DocMetadata, DocMetadataArgument, DocMetadataExample } from "../../types"
...
const renderDescription = ({
  ...
  if (docUrl) {
    ...
    descriptionLink.innerText = "Learn more" // i18n
...
const renderArg = (arg: DocMetadataArgument, highlight: boolean) => {
  ...
  if (arg.description) {
    ...
    if (arg.default && arg.optional && !arg.description.toLowerCase().includes("default")) {
      ...
      const baseText = `Defaults to ${arg.default}` // i18n
...
const renderArgs = (args: DocMetadataArgument[], highlightArgIndex?: number) => {
  ...
  argsTitle.textContent = "Parameters" // i18n
...
const renderExamples = (examples: DocMetadataExample[]) => {
  ...
  examplesTitle.textContent = "Examples" // i18n
...
export const createInfoBoxRenderer =
  ...
  (_completion: Completion, highlightArgIndex = -1) => {
    ...
    const header = isFunction ? renderFunctionHeader(doc, highlightArgIndex) : renderPropHeader(doc)
